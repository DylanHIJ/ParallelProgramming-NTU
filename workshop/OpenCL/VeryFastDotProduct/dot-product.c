...
int main(int argc, char *argv[]) {
    ...
    /* create buffer */
    cl_mem bufferVec = clCreateBuffer(
        context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR,
        MAXN * sizeof(uint32_t) / (CHUNKSIZE * LOCALSIZE), vec, &status);
    assert(status == CL_SUCCESS);
    ...
    while (scanf("%d %" PRIu32 " %" PRIu32, &N, &key1, &key2) == 3) {
        ...
        /* kernel calculation */
        size_t globalWorkSizes[] = {(size_t)N / CHUNKSIZE};
        size_t localWorkSizes[] = {LOCALSIZE};
        status = clEnqueueNDRangeKernel(
            commandQueue, kernel, 1, NULL, globalWorkSizes, localWorkSizes, 0, NULL, NULL
        );
        assert(status == CL_SUCCESS);

        /* read result from device */
        status = clEnqueueReadBuffer(
            commandQueue, bufferVec, CL_TRUE, 0,
            N * sizeof(uint32_t) / (CHUNKSIZE * LOCALSIZE), vec, 0, NULL, NULL);
        assert(status == CL_SUCCESS);

        uint32_t sum = 0;
        for (int i = 0; i < N / (CHUNKSIZE * LOCALSIZE); i ++)
            sum += vec[i];
